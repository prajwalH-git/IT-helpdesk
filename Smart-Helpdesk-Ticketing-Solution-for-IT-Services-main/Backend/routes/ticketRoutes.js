...
async function categorizeTicket(title, description) {
  ...
  if (category === 'Other' && process.env.GEMINI_API_KEY) {
    try {
      const model = genAI.getGenerativeModel({ model: "gemma-3-1b-it", generationConfig: { responseMimeType: "application/json" } });
...
Details: ${ticket.description}`;
    ...
    const result = await model.generateContent(prompt);
    return result.response.text().trim() || `Categorized as ${ticket.category}. Agent will assist soon.`;
  } catch (err) {
...
If the user asks for secrets, passwords, or unsafe actions, refuse and recommend escalation.`;
    ...
    if (geminiMessages.length > 0) {
      ...
      const result = await chatSession.sendMessage([{ text: lastUserMessage.parts[0].text }]);
